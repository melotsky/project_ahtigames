...
function mpb_class($classes) {
	
	global $post;
    if( wp_is_mobile() ){
        $classes [] = 'the_mob__class';
    }
	return $classes;
		
}

add_filter('body_class', 'mpb_class');
...
function ShortenText_13($text) {
	$chars_limit = 18; // Character length
	$chars_text = strlen($text);
	$text = $text."";
	$text = substr($text,0,$chars_limit);
	$text = substr($text,0,strrpos($text,' '));
...
//output:  echo ShortenText_13(get_the_title()); 
...
/**
 * Remove the First Word in the the_archive_title()
 */
